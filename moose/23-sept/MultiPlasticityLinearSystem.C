 ...
 */
  ...
  Real H = 2e5; // = ??? picked a number ~young's mod
  Real yield_strength = 2.42e2; // until I can figure out how to access from user input

  // compute deviatoric trial stress = stress - 1/3 tr stress * I
  ...
  std::vector<Real> jac; jac.size()=1; // del_lambda_k in book (dpm in code)
  dpm[0] = jac[0] = (eff_trial_stress - 3 * mu * pm - yield_strength) / (3 * mu + H);
  // sassuming sigma_y_bar(eff_strain) is a single quantity, not a product
  ...
  //from other file (to save in case needed again)
  // compute deviatoric trial stress = stress - 1/3 tr stress * I
 ...
 */
 ...
 /*
  ...
  for (unsigned column = 0 ; column < row ; ++column)
  mag_unit += pow(dev_trial_stress[row][column], 2);
  mooseAssert(mag_unit < 1.01 && mag_unit > 0.99, "Incorrect calculation of unit direction normal to yield surface.");
 */
}
