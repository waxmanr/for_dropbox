...
ComputeMultiPlasticityStress::computeQpStress()
{
  ...
  _elastic_strain[_qp] = _elastic_strain_old[_qp] + _my_strain_increment - (_plastic_strain[_qp] - _plastic_strain_old[_qp]);
...
ComputeMultiPlasticityStress::plasticStep(const RankTwoTensor & stress_old, RankTwoTensor & stress, const std::vector<Real> & intnl_old, std::vector<Real> & intnl, const RankTwoTensor & plastic_strain_old, RankTwoTensor & plastic_strain, const RankFourTensor & E_ijkl, const RankTwoTensor & strain_increment, std::vector<Real> & yf, unsigned int & iterations, bool & linesearch_needed, bool & ld_encountered, bool & constraints_added, RankFourTensor & consistent_tangent_operator)
{
  ...
  {
    iter = 0;
    bool radialReturn = true;
    if (radialReturn)
      return_successful = radialReturnMap(stress_good, stress, intnl_good, intnl, plastic_strain_good, plastic_strain, E_ijkl, dep, yf, iter, step_size <= _max_stepsize_for_dumb, linesearch_needed, ld_encountered, constraints_added, time_simulated + step_size >= 1, consistent_tangent_operator, cumulative_pm);
    else
...
ComputeMultiPlasticityStress::returnMap(const RankTwoTensor & stress_old, RankTwoTensor & stress, const std::vector<Real> & intnl_old, std::vector<Real> & intnl, const RankTwoTensor & plastic_strain_old, RankTwoTensor & plastic_strain, const RankFourTensor & E_ijkl, const RankTwoTensor & strain_increment, std::vector<Real> & f, unsigned int & iter, const bool & can_revert_to_dumb, bool & linesearch_needed, bool & ld_encountered, bool & constraints_added, const bool & final_step, RankFourTensor & consistent_tangent_operator, std::vector<Real> & cumulative_pm)
{
  ...
  while (still_finding_solution)
  {
    ...
    if (!nr_good)
    {
      ...
      if (change_scheme)
        changeScheme(initial_act, can_revert_to_dumb, initial_stress, intnl_old,
                     deact_scheme, act, dumb_iteration, dumb_order);
    ...
    if (nr_good)
    {
      ...
      if (!kt_good)
      {
        if (deact_scheme != dumb)
        {
          ...
          still_finding_solution = (actives_tried.find(activeCombinationNumber(act)) == actives_tried.end());
          if (!still_finding_solution)
          {
...
bool
ComputeMultiPlasticityStress::radialReturnMap(const RankTwoTensor & stress_old, RankTwoTensor & stress, const std::vector<Real> & intnl_old, std::vector<Real> & intnl, const RankTwoTensor & plastic_strain_old, RankTwoTensor & plastic_strain, const RankFourTensor & E_ijkl, const RankTwoTensor & strain_increment, std::vector<Real> & f, unsigned int & iter, const bool & can_revert_to_dumb, bool & linesearch_needed, bool & ld_encountered, bool & constraints_added, const bool & final_step, RankFourTensor & consistent_tangent_operator, std::vector<Real> & cumulative_pm)
{
  bool successful_return = elasticStep(stress_old, stress, intnl_old, intnl, plastic_strain_old, plastic_strain, E_ijkl, strain_increment, f, iter, consistent_tangent_operator);
  ...
  // Plastic strain & internal constraints, just in case (don't know if needed)
  RankTwoTensor epp;
  std::vector<Real> ic;

  // Save initial value to revert in case of failure
  RankTwoTensor initial_stress = stress;

  iter = 0;

  // Don't think this is needed, as there's only one f > f_tol (j2 / von mises yf)
  // act is scalar?
  std::vector<bool> act;
  buildActiveConstraints(f, stress, intnl, E_ijkl, act);
  yieldFunction(stress, intnl, act, f); //no idea if this is needed either

  // change in plastic strain, init to 0
  RankTwoTensor delta_dp = RankTwoTensor();

  // consistency parameters, plastic multipliers, deltaLambda in book
  // scalar, as only one surface
  std::vector<Real> pm(0.0);

  bool single_step_success = true;

  // skipping scheme building, as there is only one surface

  // residual-squared from NR iteration\
  // don't need loop or if, as only one surface, and always active within return map
  Real nrRad_res2 = 0;
  nrRad_res2 += 0.5*std::pow(f[0]/_f[modelNumber(0)]->_f_tol, 2.0);
  ...
  while (still_finding_solution)
  {
    ...
    // Newton-Raphson radial return loop
    while (nrRad_res2 > 0.5 && local_iter++ < _max_iter && single_step_success)
      single_step_success = singleStepRadial(nrRad_res2, stress, intnl_old, intnl, pm,
                                       delta_dp, E_ijkl, f, epp, ic, act, deact_scheme,
                                       linesearch_needed, ld_encountered);

    bool nr_good = (nrRad_res2 <= 0.5 && local_iter <= _max_iter && single_step_success);

    iter += local_iter;

    //skip change scheme section

    bool kt_good = false;
    if (nr_good)
      kt_good = checkKuhnTucker(f, pm, act); //should be able to keep this part the same

    //again skipping schemes

    bool admissible = false;
    if (nr_good && kt_good)
      admissible = true; //single surface
    ...
    if (still_finding_solution)
    {
      ...
      for (unsigned model = 0 ; model < _num_models ; ++model)
        intnl[model] = intnl_old[model];  // back to old internal params
      pm(0.0); // back to zero plastic multipliers

      yieldFunction(stress, intnl, act, f); // reset with old stress

      // no need for if statement here, as there are no other active surfaces
      successful_return = false;
        break; // failure
    }
    ...
    nrRad_res2 = 0;
    nrRad_res2 += 0.5*std::pow(f[0]/_f[modelNumber(0)]->_f_tol, 2.0);
  } //end while(still finding solution)
                                              ...
                                              const bool & can_revert_to_dumb)
{
  ...
  if (   current_deactivation_scheme == safe
...
bool
ComputeMultiPlasticityStress::singleStepRadial(Real & nr_res2,
                                         RankTwoTensor & stress,
                                         const std::vector<Real> & intnl_old,
                                         std::vector<Real> & intnl,
                                         std::vector<Real> & pm,
                                         RankTwoTensor & delta_dp,
                                         const RankFourTensor & E_ijkl, std::vector<Real> & f,
                                         RankTwoTensor & epp,
                                         std::vector<Real> & ic,
                                         std::vector<bool> & active,
                                         DeactivationSchemeEnum deactivation_scheme,
                                         ...
                                         bool & ld_encountered)
{
  ...
  std::vector<Real> intnl_before_step = intnl;
  std::vector<Real> pm_before_step = pm;
  RankTwoTensor delta_dp_before_step = delta_dp;
  */

  RankTwoTensor dstress; // change in stress
  std::vector<Real> dpm; // plasticity mulitplier (should be scalar)
  std::vector<Real> dintnl; // change in internal parameters, also scalar

  // not used but needed to pass as param
  std::vector<bool> deact_ld;
  deact_ld.assign(_num_surfaces, false);

  //should not need while (constraints_changing) loop, as constraints don't change
  ...
  nrRad_res2 = residual2(pm, f, epp, ic, active, deactivated_due_to_ld);
...
bool
ComputeMultiPlasticityStress::checkAdmissible(const RankTwoTensor & stress, const std::vector<Real> & intnl, std::vector<Real> & all_f)
{
  std::vector<bool> act;
  act.assign(_num_surfaces, true);
...
void
ComputeMultiPlasticityStress::applyKuhnTucker(const std::vector<Real> & f, const std::vector<Real> & pm, std::vector<bool> & active)
{
...
ComputeMultiPlasticityStress::consistentTangentOperator(const RankTwoTensor & stress, const std::vector<Real> & intnl, const RankFourTensor & E_ijkl, const std::vector<Real> & pm_this_step, const std::vector<Real> & cumulative_pm)
{
  ...
  // in tensor mechanics:
  // stress_coeff(i, j, k, l) = stress_coeff(j, i, k, l) = stress_coeff(i, j, l, k) != stress_coeff(k, l, i, j)
