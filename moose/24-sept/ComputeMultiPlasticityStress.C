...
InputParameters validParams<ComputeMultiPlasticityStress>()
{
  ...
  //params.addParam<bool>("isotropic_elastic", false, "Isotropic case where von mises stress can be used --> radial return mapping");
...
ComputeMultiPlasticityStress::ComputeMultiPlasticityStress(const InputParameters & parameters) :
    ...
    //_isotropic_elastic(getParam<bool>("isotropic_elastic")),
...
/*bool ComputeMultiPlasticityStress::radialReturnMap(const RankTwoTensor & stress_old, RankTwoTensor & stress)
{
}*/
...
ComputeMultiPlasticityStress::singleStep(Real & nr_res2,
                                         ...
                                         const RankFourTensor & E_inv, const RankFourTensor & E_ijkl,
                                         std::vector<Real> & f,
                                         RankTwoTensor & epp,
                                         std::vector<Real> & ic,
                                         ...
                                         bool & ld_encountered)
{
  ...
  {
    // check to see if radialReturnMapping can be used to simplify solution
    bool radialReturnMap = false;
    /*if (_isotropic_elastic)
      radialReturnMap = true;
    else if (ComputeElasticityTensor::RankFourTensor::FillMethod == symmetric_isotropic || ComputeElasticityTensor::RankFourTensor::FillMethod == general_isotropic || ComputeElasticityTensor::RankFourTensor::FillMethod == antisymmetric_isotropic)
      radialReturnMap = true;*/
    //other else to account for user-defined isotropic elasticity (no fill method)
    //for now just declaring true to test
    radialReturnMap = true;

    // calculate dstress, dpm and dintnl for one full Newton-Raphson step
    if (radialReturnMap)
      nrStepRadial(stress, intnl_old, intnl, pm, E_inv, delta_dp, dstress, dpm, dintnl, active, deact_ld);
    else
      nrStep(stress, intnl_old, intnl, pm, E_jkl, delta_dp, dstress, dpm, dintnl, active, deact_ld);
