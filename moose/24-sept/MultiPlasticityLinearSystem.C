...
int
MultiPlasticityLinearSystem::singularValuesOfR(const std::vector<RankTwoTensor> & r, std::vector<Real> & s)
{
  int bm = r.size();
  int bn = 6;
...
MultiPlasticityLinearSystem::calculateJacobian(const RankTwoTensor & stress, const std::vector<Real> & intnl, const std::vector<Real> & pm, const RankFourTensor & E_inv, const std::vector<bool> & active, const std::vector<bool> & deactivated_due_to_ld, std::vector<std::vector<Real> > & jac)
{
  ...
  std::vector<Real> df_dintnl; // df_dq in book
  dyieldFunction_dintnl(stress, intnl, active_surface, df_dintnl);

  std::vector<RankTwoTensor> r; // psi in book
  flowPotential(stress, intnl, active, r);

  std::vector<RankFourTensor> dr_dstress; // r in book
  dflowPotential_dstress(stress, intnl, active, dr_dstress);

  std::vector<RankTwoTensor> dr_dintnl; // dpsi_dq in book
  ...
  // d(epp)/dstress = sum_{active alpha} lambda_dot[alpha]*r (in book)
...
void
MultiPlasticityLinearSystem::nrStep(const RankTwoTensor & stress, const RankTwoTensor & strain, const std::vector<Real> & intnl_old, const std::vector<Real> & intnl, const std::vector<Real> & pm, const RankFourTensor & E_inv, const RankFourTensor & E_ijkl, const RankTwoTensor & delta_dp, RankTwoTensor & dstress, std::vector<Real> & dpm, std::vector<Real> & dintnl, const std::vector<bool> & active, std::vector<bool> & deactivated_due_to_ld)
{
...
{// if (radialReturnMap)
  // calc 1x1 jac == del_lambda
  // use fk from RHS --- check sign, might need -fk --- how is this extraced from RHS?
  // get del_lambda (dpm) from scalar/tensor equation using 1x1 jac
  // calculate dstress, dq
  // try without changing linesearch, resid, or calcConstraints
  // hoping irrelevant values will go to zero and work themselves out

  // calculate RHS to extract f
  std::vector<Real> rhs;
  calculateRHS(stress, intnl_old, intnl, pm, delta_dp, rhs, active, true, deactivated_due_to_ld);

  // taken from fill method isosym
  ...
  // yield function f = 7th comp of rhs
  dpm = -rhs[6] / (3 * mu + df_dintnl);
  delta_dp = - jac * sqrt(1.5) * unit_normal;
  dstress = 2 * mu * delta_dp;
  dintnl = dpm; // only internal variable is hardening; q1 = pm (lambda in book)
  // strain_new = strain_k + jac; NEED TO FIND


  //notes saved from today:
  //DELETED the following because delta_dp is passed as const, which means it doesn't
  //need to be changed here

  /*// need to calculate stress_dev in order to get unit_normal for delta_dp
  RankTwoTensor dev_trial_stress(stress); // should that be stress, instead of stress_new?
  ...
  Real norm_dev_stress = 0;
  for (unsigned row = 0 ; row < 3 ; ++row)
    for (unsigned column = 0 ; column < row ; ++column)
      norm_dev_stress += pow(dev_trial_stress(row,column), 2);

  norm_dev_stress = std::sqrt(norm_dev_stress);

  RankTwoTensor unit_normal;
  unit_normal = dev_trial_stress / norm_dev_stress;*/

  // yield function f = 7th comp of rhs
  //dpm[0] = -rhs[6] / (3 * mu - df_dintnl[0]);
  //delta_dp = unit_normal * (-dpm[0] * std::sqrt(1.5));
